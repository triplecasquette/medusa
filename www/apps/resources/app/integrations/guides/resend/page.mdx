...
For development purposes, Resend allows you to send emails from the email `onboarding@resend.dev` and only to your account's email. If you have a custom domain that you want to send emails from, you must add it to your Resend account's domains:
...
In this step, you'll install two dependencies useful for your Resend integration:
...
You'll use these dependencies in the next sections.
...
Medusa has a Notification Module that acts as the interface to send notifications to users and customers through any channel, such as email or SMS. The actual sending is handled by Notification Module Providers that you can add to the Notification Module. Module providers are essentially modules, but are registered as part of another module, such as the Notification Module, rather than on their own.

In this step, you'll create a Resend Module Provider that implements sending notifications through the email channel. In later steps, you'll send email notifications when an order is placed through Resend.
...
In this section, you'll create the Resend Module's service and the methods necessary to send an email with Resend.
...
A Notification Module Provider must extend the `AbstractNotificationProviderService`. You'll soon implement its `send` method in `ResendNotificationProviderService`. The provider must also have an `identifier` static property, which is a unique identifier that Medusa will use when registering the provider in the database.

The `ResendNotificationProviderService` also has the following properties:
...
- `options` of type `ResendOptions`. Modules accept options through Medusa's configurations. This ensures that the module is reusable across applications and you don't use sensitive variables like API keys directly in your code. The options that the Resend Module Provider accepts are:
    ...
    - `from`: The email to send the emails from.
...
Notification Module Providers can implement a static `validateOptions` method that ensures the options passed to the module through Medusa's configurations are valid.
...
Before sending an email, you need to retrieve its template. For example, order confirmation emails can have one template, whereas customer registration confirmation emails have another.
...
The `send` method receives the notification details as a parameter. Some of the object's properties include:

- `to`: The address to send the notification to.
- `template`: The template type of the notification.
- `data`: The data useful for the email type. For example, if the email is for order confirmation, `data` would hold the order's details.

In the method, you retrieve the template and subject of the email using the methods you defined earlier. Then, you put together the data to pass to Resend, such as the email to send the notification to and the from email. Also, if the email's template is a string, it's passed as an HTML template. Otherwise, it's passed as a React template.
...
You export the module provider's definition using the `ModuleProvider` utility function, which accepts as a first parameter the name of the module that this provider belongs to, which is the Notification Module in this case. It also accepts as a second parameter an object having a `service` property, indicating the module's service.
...
![The form to create an API key with fields for the API key's name, permissions, and domain](https://res.cloudinary.com/dza7lstvk/image/upload/v1732535464/Medusa%20Resources/Screenshot_2024-11-25_at_10.23.26_AM_g7gcuc.png)
    ...
    - A new pop-up will show with your API key hidden. Copy it before closing the pop-up, as you can't access the key again. Use its value for the `RESEND_API_KEY` environment variable.
...
You define the `OrderPlacedEmailComponent` which is a React component that shows the order's details, such as items and their totals, in the email. The component accepts the order as a prop.

You also export an `orderPlacedEmail` function, which accepts props as an input and returns the `OrderPlacedEmailComponent` passing it the props. This is because you can't use JSX syntax in `src/modules/resend/service.ts`.
...
Instead, you create a subscriber that listens to the `order.placed` event and sends an email when the event is emitted, which is what you'll do in this step.
...
You define the `sendNotificationStep` using the `createStep` function, which accepts two parameters:

- A string indicating the step's unique name.
- The step's function definition as a second parameter. It accepts the step's input as a first parameter, and an object of options as a second that has a `container` property.
...
```ts title="src/workflows/send-order-confirmation.ts"
...
1. `useQueryGraphStep`, which is a step implemented by Medusa that uses [Query](!docs!/learn/advanced-development/module-links/query), a tool that allows you to retrieve data across modules, to retrieve the order's details.
2. `sendNotificationStep` which is the step you implemented. You pass it an array having one object, which is the notification's details having following properties:
    - `to`: The address to send the email to. You pass the customer's email that is stored in the order.
    - `channel`: The channel to send the notification through, which is `email`. Since you also specified `email` in the Resend Module Provider's options, the Notification Module will delegate the sending to the Resend Module Provider's service.
...
- A configuration object with an `event` property whose value is the event the subscriber is listening to.
...
This subscriber now runs whenever an order is placed, which you'll test out in the next step.
...
Then, in the Next.js storefront's directory (which was installed outside of the Medusa application's directory with the name `{project-name}-storefront`, where `{project-name}` is the name of the Medusa application's directory), run the following command to start the storefront:
...
2\. Click on a product, then choose from its options and add it to the cart.
